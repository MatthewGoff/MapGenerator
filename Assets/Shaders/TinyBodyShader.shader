	...
	SubShader
	{
		Pass
		{
			...
			fixed4 frag(fragmentInput fragInput) : SV_Target
			{
				...
				for (int i = 0; i < 100; i++)
				{
					if (i < numberOfBodies[offset] && returnColor.a != 1)
					{
						...
						if (body.w == 0)
						{
							...
							{
								float2 texturePosition = float2(fragInput.worldPos.x - body.x + body.z, fragInput.worldPos.y - body.y + body.z);
								texturePosition /= 2 * body.z;
